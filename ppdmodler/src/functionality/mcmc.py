...
def model(theta: np.ndarray, sampling: int, wavelength: float) -> np.ndarray:
    ...
    fwhm = theta
    model = Gauss2D().eval_vis(fwhm, sampling, wavelength)
    return model
...
def lnlike(theta: np.ndarray, sampling: int, wavelength: float, vis2data: np.ndarray,
    ...
    more important"""
    vis2datamod = map(lambda x: x*np.conj(x), [model(theta, sampling, wavelength)])
    vis2datamod = correspond_uv2model(vis2datamod, uvcoords, True)
...
def lnprob(theta: np.ndarray, sampling: int, wavelength: float, vis2data: np.array,
    ...
    if not lp == 0.0:
        return -np.inf
    return lp + lnlike(theta, sampling, wavelength, vis2data, vis2err)

def main(p0, nwalkers_burn, nwalkers, niter_burn, niter, ndim, lnprob, data) -> np.array:
    """"""
    ...
    # the default 
    sampler = emcee.EnsembleSampler(nwalkers_burn, ndim, lnprob, args=data)
...
if __name__ == "__main__":
    ...
    vis2data, vis2err, vis2sta = readout.get_vis2()
    uvcoords = readout.get_uvcoords()

    # Correspond uv-coords to indicies of the model with certain size/sampling
    # rate
    ...
    nwalkers_burn, nwalkers, ndim = 250, 50, len(initial)
    ...
    sampler, pos, prob, state = main(p0, nwalkers_burn, nwalkers, niter_burn, niter, ndim, lnprob, data)
